...
export const AST_NODES = {
    ...
    ImportAttribute: {
        estreeType: "{ key: estree.Identifier | estree.Literal; type: 'ImportAttribute'; value: estree.Literal; }",
        fields: [
            ['key', 'Node'],
            ['value', 'Node']
        ],
...
export const astNodeNamesWithFieldOrder = Object.entries(AST_NODES).map(([name, originalNode]) => {
    // This functions adds Some Keys as node to AST_NODE[AST_NODE_KEY].node if hasSame... and copy to originalNode
    const AST_NODES_KEY = originalNode.hasSameFieldsAs;
    const node = AST_NODES_KEY ? AST_NODES[AST_NODES_KEY] : originalNode;
    return {
        fields: node.fields || [],
        name,
        node,
